...
#'
#' \code{LPM_AIC_MCV_50Split} estimates the error variance with the test set RMSE
#'
#' @param XMat a n by p numeric matrix
#' @param yVec a n by 1 numeric vector
#' @param numCVOuter number of best AIC model splits
#' @param numCVInner number of inner cross validation splits (default is 100)
#'
...
#' \item MCV: we find the LPM on multiple training sets, and get RMSE from the test set
...
LPM_AIC_CV_50Split = function(XMat,yVec,
  ...
  mseVec <- rep(0,times=numCVOuter)
  
  
  for (i in 1:numCVOuter) {
    ...
    tempMSEVec <- rep(0,times=numCVInner)
    for (j in 1:numCVInner) {
      ...
      tempMSEVec[j] <- mean((testY - tempPred)^2,na.rm=TRUE)
    }
    mseVec[i] <- mean(tempMSEVec)
  }
