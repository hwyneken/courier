...
#' If there are linear dependencies amongst the columns, some of the diagonal entries will be zero.
#' A small practical example will help to explain this.
#' 
#' Suppose X <- cbind(c(1,1,0),c(0,0,1),c(1,1,1)).
#' Therefore, the third column is just the sum of the first and second columns.
#' The output from \code{pracma::rref(X)} is:
#' 
#' rref(X)
#'      [,1] [,2] [,3]
#; [1,]    1    0    1
...
#' [3,]    0    0    0
#' 
#' The leading 1's on the diagonal indicate that we will keep the first two columns.
#' However, the third column can be written as a sum of the first and second columns, 
#'  as shown by the value of the third column: (c(1,1,0)).
